...
const Pagination = ({totalPages}:{totalPages:number}) => {
  ...
  const PaginationNumber = ({
    ...
    const className = clsx("flex h-10 w-10 items-center justify-center text-sm text-white border",{
        "rounded-l-sm": position === "first" || position === "single",
        "rounded-r-sm": position === "last" || position === "single",
        "z-10 bg-gray-600 border-white text-white":isActive,
  ...
  const PaginationArrow = ({
    ...
    isDisabled:boolean;
     ...
     const className = 
     clsx("flex h-10 w-10 items-center justify-center text-sm border text-white",{
        "pointer-events-none text-gray-300 bg-gray-400":isDisabled,
